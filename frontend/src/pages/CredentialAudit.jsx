...
const CredentialAudit = () => {
  ...
  const handleAuditSubmit = async (values) => {
    try {
      await personnelApi.audit(currentItem.id, values);
  ...
  const getTypeLabel = (type) => {
    const map = { exhibitor: '展商证', worker: '工作人员证', vip: 'VIP证', media: '媒体证' };
  ...
  const columns = [
    { 
      ...
      width: 80,
      render: (photo) => photo ? (
        <Image width={50} height={60} src={photo} style={{ objectFit: 'cover', borderRadius: 4 }} />
      ) : <Tag color="default">无</Tag>
    },
    { title: '姓名', dataIndex: 'name', key: 'name', width: 100 },
    { title: '身份证号', dataIndex: 'id_card', key: 'id_card', width: 180 },
    { title: '公司', dataIndex: 'company_name', key: 'company_name' },
    { title: '证件类型', dataIndex: 'credential_type', key: 'credential_type', width: 120, render: getTypeLabel },
  ...
  return (
    <div>
      <h2 className="page-title">证件审核</h2>
        ...
        {currentItem && (
          <>
            ...
            <Descriptions bordered column={1} size="small" style={{ marginBottom: 16 }}>
              ...
              <Descriptions.Item label="证件类型">{getTypeLabel(currentItem.credential_type)}</Descriptions.Item>
            </Descriptions>
        ...
        {currentItem && (
          <>
            ...
            <Descriptions bordered column={1}>
              ...
              <Descriptions.Item label="审核状态">{getStatusTag(currentItem.audit_status)}</Descriptions.Item>
            </Descriptions>
          </>
        )}
      </Drawer>
